# app/progress_detail_container/progress_detail_container_models.py
# DB에 저장될 사용자 정보를 정의하는 ORM 모델

from sqlalchemy import Column, Integer, DateTime, ForeignKey, String, ARRAY, Boolean, Float,Date
...
class ProgressContainer(Base):
    __tablename__ = 'progress_detail_container'
    ...
    SHORING = Column(Integer,nullable=True)
    TRUCKING = Column(Integer,nullable=True)
    THC = Column(Integer, nullable=True)
    ...
    progress = relationship('Progress', back_populates='progress_detail_container', passive_deletes=True)

    progress_detail_container_detail = relationship('ProgressContainerDetail', back_populates='progress_detail_container', cascade='all, delete-orphan',
    ...
    creator = relationship('User', back_populates='progress_detail_container',passive_deletes=True)  # 유저가 삭제되어도 db에 posts가 남음
...
class ProgressContainerDetail(Base):
    __tablename__ = 'progress_detail_container_detail'
    id = Column(Integer, primary_key=True, index=True)
    ...
    progress_detail_container_id=Column(Integer,ForeignKey('progress_detail_container.id',ondelete='CASCADE'),nullable=True)
    progress_detail_container = relationship('ProgressContainer',back_populates='progress_detail_container_detail',passive_deletes=True)
