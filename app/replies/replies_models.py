...
class Reply(Base):
    ...
    creator_id = Column(Integer, ForeignKey('users.id',ondelete='SET_NULL'),nullable=True) # 유저가 삭제되어도 replies가 남음 ondelete='SET NULL'을 추가하면 삭제된 유저의 아이디는 null로 표시됨, null 이 됐을때 오류 방지를 위해 nullable=True 를 써줌
    creator = relationship('User',backref=backref('replies'),passive_deletes=True) # passive_deletes 는 FK의 ondelete에 따름 (SET_NULL = 연결된 객체 삭제시 삭제 안되고 NULL이 됨)
    shipment_id = Column(Integer, ForeignKey('shipments.id',ondelete='CASCADE')) #shipments table의 id 컬럼을 참조, CASCADE 게시글이 삭제되면 리플도 삭제
    shipment = relationship('Shipment', backref=backref('replies'), passive_deletes=True) # passive_deletes 는 FK의 ondelete에 따름 (CASCADE = 연결된 객체 삭제시 같이 삭제됨)
