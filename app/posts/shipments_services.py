...
from fastapi import  Depends, HTTPException, UploadFile, File, Form, responses  # FastAPI 관련 각종 import (의존성, 파일업로드, 예외처리, 응답 등)
...
class ShipmentsServices:
    ...
    async def list_shipment_personal(
            self,
            page: int = 1,  # page 를 기본값을 1을줌
            size: int = 10,  # 리스트 사이즈를 10개를줌
            type_category: int = None,
        ...
        base_query = select(shipments_models.Shipment).where(shipments_models.Shipment.creator_id == current_user.id)  # 선적(게시글) 전체 SELECT 쿼리 생성
        ...
        if search:
            ...
            total_count_query = select(func.count()).select_from(shipments_models.Shipment).where(shipments_models.Shipment.creator_id == current_user.id)
        total_count = await self.db.scalar(total_count_query)  # 전체 게시글 개수 집계
    ...
    async def update_shipment(
        ...
        shipment = await self.db.get(shipments_models.Shipment,ship_id)  # DB에서 Shipment 테이블에서 해당 ID의 게시글 1개 조회 (없으면 None 반환)
    ...
    async def delete_shipment(
        ...
        shipment = await self.db.get(shipments_models.Shipment,
                                ship_id)  # DB에서 Shipment 테이블의 기본키가 ship_id인 레코드를 조회함 (없으면 None 반환)

        if not shipment:  # shipment가 None이면 → 존재하지 않는 게시글
            raise HTTPException(status_code=404, detail="Shipment not found")  # HTTP 404 에러 발생 (게시글을 찾을 수 없음)
    ...
    async def download_file(
        ...
        shipment = await self.db.get(shipments_models.Shipment, ship_id)  # ship_id로 단일 게시글 조회
