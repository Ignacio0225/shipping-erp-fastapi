...
class Shipment(Base):
    ...
    type_category = relationship('TypeCategory',back_populates='shipments',passive_deletes=True)

    region_category_id = Column(Integer,ForeignKey('region_categories.id',ondelete='SET NULL'),nullable=True)
    region_category = relationship('RegionCategory', back_populates='shipments', passive_deletes=True)
    ...
    creator = relationship('User', back_populates='shipments', passive_deletes=True)  # 유저가 삭제되어도 db에 shipments가 남음

    reply = relationship('Reply',back_populates='shipments')
