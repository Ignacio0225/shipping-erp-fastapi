...
import math  # [주석추가] 수학 함수(ceil 등) 사용을 위해 import
import shutil, os  # [주석추가] 파일 복사/삭제(shutil), 경로 생성/조작(os)용 모듈
import uuid  # [주석추가] 파일명 유니크하게 할 때 사용하는 UUID 생성기

from typing import Optional  # [주석추가] 파라미터/타입 어노테이션에 Optional 사용

from fastapi import APIRouter, Depends, HTTPException, UploadFile, File, Form, responses, Query  # [주석추가] FastAPI 관련 각종 import (의존성, 파일업로드, 예외처리, 응답 등)
from pathlib import Path  # [주석추가] 파일 경로 객체로 변환, exists 체크용
from sqlalchemy.ext.asyncio import AsyncSession  # [주석추가] 비동기 SQLAlchemy 세션
from sqlalchemy import select, update, delete, func, or_  # [주석추가] SQL 쿼리 빌더, 함수, OR 검색 등
from sqlalchemy.orm import selectinload  # [주석추가] 관계형 데이터 JOIN/프리패치용

from app.users import users_models, users_schemas  # [주석추가] 사용자 ORM/스키마
from app.posts import shipments_models, Shipment  # [주석추가] 선적 ORM 및 Shipment 엔티티
from app.posts import shipments_schemas  # [주석추가] 선적 스키마
from app import database  # [주석추가] DB 세션 의존성
from app.users import dependencies  # [주석추가] 로그인/권한 의존성
...
async def list_shipment(
    ...
    if size < 1:  # 리스트가 9개 8 개 7개 올수도 있음 하지만 음수나 0이 나오면 다시 10으로 만들어버림
        size = 10
    offset = (page - 1) * size  # [주석추가] OFFSET 계산(몇 개를 건너뛸지)
    ...
    base_query = select(shipments_models.Shipment)  # [주석추가] 선적(게시글) 전체 SELECT 쿼리 생성
    ...
    base_query = base_query.options(selectinload(shipments_models.Shipment.creator))  # [주석추가] 작성자 정보(creator)를 JOIN 해서 한 번에 가져오도록 설정

    result = await db.execute(base_query)  # [주석추가] 쿼리 실행해서 결과 받아옴
    ...
    items = [
        ...
        for s in shipments  # [주석추가] 모든 shipments(게시글) 객체를 Pydantic 스키마로 변환, 작성자 정보 포함
    ]
    ...
    total_count = await db.scalar(total_count_query)  # [주석추가] 전체 게시글 개수 집계
...
async def get_shipment(
    ...
    res = await db.get(shipments_models.Shipment, ship_id)  # [주석추가] ship_id로 해당 게시글 단건 조회
    if not res:
        raise HTTPException(status_code=404, detail='shipment not found')  # [주석추가] 없는 경우 404 반환
    return res  # [주석추가] (여기선 작성자 정보까지 제대로 반환하려면 별도 selectinload 필요, 단건 상세라면 추가로 구현해도 됨)
...
async def create_shipment(
    ...
    payload = shipments_schemas.ShipmentCreate(title=title, description=description)  # [주석추가] 입력값을 Pydantic 모델로 생성

    file_paths = None  # [주석추가] 파일이 없을 때 None(Null)로 저장

    if files:  # 파일이 첨부된 경우에만 아래의 코드 실행
        os.makedirs(UPLOAD_DIR, exist_ok=True)  # [주석추가] 업로드 폴더 없으면 새로 만듦

        new_file_paths = []  # [주석추가] 저장할 파일 경로들 담을 리스트
        ...
        for file in files:
            ...
            new_file_paths.append(saved_path)  # [주석추가] 저장한 경로 리스트에 추가
        file_paths = new_file_paths  # [주석추가] 최종 저장 경로 리스트로 교체
...
async def update_shipment(
    ...
    payload = shipments_schemas.ShipmentUpdate(title=title, description=description)  # [주석추가] 수정 입력값을 Pydantic 모델로
    ...
    file_paths = list(keep_set)  # [주석추가] (삭제 안된) 파일 경로만 리스트로 변환

    if new_file_paths:  # 업로드 된 파일이 있으면
        # 파일이 한 개만 넘어오면 리스트로 감싸줌
        if isinstance(new_file_paths, UploadFile):  # [주석추가] 단일 파일일 때 리스트로 변환
            new_file_paths = [new_file_paths]
        for file in new_file_paths:  # for 문을 돌림
            os.makedirs(UPLOAD_DIR, exist_ok=True)  # [주석추가] 업로드 폴더 없으면 생성
            saved_path = os.path.join(UPLOAD_DIR, f"{uuid.uuid4()}_{file.filename}")  # [주석추가] 새 파일 저장 경로
            ...
            file_paths.append(saved_path)  # [주석추가] 새 파일 경로 추가
    ...
    await db.commit()  # [주석추가] 트랜잭션 커밋
    await db.refresh(ship)  # [주석추가] 갱신/재조회
    return ship  # [주석추가] 수정된 ship 반환
...
async def delete_shipment(
    ...
    await db.execute(delete(shipments_models.Shipment).where(shipments_models.Shipment.id == ship_id))  # [주석추가] 삭제 쿼리 실행
    await db.commit()  # [주석추가] 실제 DB 반영
...
async def download_file(
    ...
    shipment = await db.get(shipments_models.Shipment, ship_id)  # [주석추가] ship_id로 단일 게시글 조회

    # 해당 게시글 조회
    if not shipment:
        raise HTTPException(status_code=404, detail='shipment not found')  # [주석추가] 없는 경우 예외

    # 인덱스 범위 확인
    try:
        file_path_str = shipment.file_path[file_index]  # [주석추가] 인덱스에 해당하는 파일 경로 추출
    ...
    # 파일 존재 여부 확인
    file_path = Path(file_path_str)  # [주석추가] 문자열을 Path 객체로 변환
